---
title: "ASSIGNMENT 3 SUBMISSION BY A.ANANDHABALA"
author: "ANANDHABALA"
date: "2022-10-10"
output: html_document
---
# dowload the require packages.
```{r}
library(ISLR)
library(MASS)
attach(Default)
...
### get the auto data###
attach(Auto)
### all data set 0 using rep commend ###
mpg01 <- rep(0, length(mpg))
### another value set based on a median set for a  1 ### 
mpg01[mpg > median(mpg)] <- 1 
Auto_mpg01 <- data.frame(Auto, mpg01)
### find the numer of observation using dim ###
dim(Auto_mpg01)
### we need ignore first 9 observation using head command ###
head(Auto_mpg01[, -9])
attach(Auto_mpg01)
```

Ans : create a mpg01 and Assign 1 if mpg is above the median then combine Auto and mpg01
...
### exclude the train data that data is test data  ###  
mpg01.test <- mpg01[!train]
```
Ans : Split by even years The Train Data have 210 records The Test Data have 182 records
  
(d)	Perform LDA on the training data in order to predict mpg01 using the variables that seemed most associated with mpg01 in (b). What is the test error of the model obtained? 

```{r}
### model build by mpg data ###
...
```{r}
### model build for auto data predictor is cylinder,weight,displacement,horsepower
qda.fit <- qda(mpg01 ~ cylinders + weight + displacement + horsepower, 
               data = Auto_mpg01, subset = train)
qda.fit
### find the probility using predict commend
...
With KNN Analysis (K = 5), the test error rate is `r mean(knn.pred.5 != mpg01.test) * 100`%
                ...
                ### ASSIGNMENT 3 SUBMISSION BY A.ANANDHABALA ###